...
char Game::getState(int row, int col) const {
    if (this->board[row][col]) { // if there exists a player on the board
        return this->board[row][col]->getPieceType();
    }
    return '$';
}

bool Game::pawnPromotionMove( vector<vector<unique_ptr<Piece>>> &board, Move moveToPlay) {
...
void Game::updateEnPassant( vector<vector<unique_ptr<Piece>>> &board, Move moveToPlay) {
    if(board[moveToPlay.getFromX()][moveToPlay.getFromY()] == nullptr) {
      ...
      if (board[moveToPlay.getFromX()][moveToPlay.getFromY()] != nullptr && board[moveToPlay.getFromX()][moveToPlay.getFromY()]->getPieceType() == 'p' && moveToPlay.getFromX() == 1 && moveToPlay.getToX() == 3)
            ...
            {
                Pawn* pawn = dynamic_cast<Pawn*>(board[moveToPlay.getToX()][moveToPlay.getToY() + 1].get());
                pawn->setPassant("left", true);
                // board[moveToPlay.getToX()][moveToPlay.getToY() + 1]->setPassant("left", true);
                // return true;
...
bool Game::executeEnPassant( vector<vector<unique_ptr<Piece>>> &board, Move moveToPlay) {
     if ((moveToPlay.getFromX() == 3 || moveToPlay.getFromX() == 4) && tolower(board[moveToPlay.getFromX()][moveToPlay.getFromY()]->getPieceType()) == 'p' && moveToPlay.getToY() != moveToPlay.getFromY() && board[moveToPlay.getToX()][moveToPlay.getToY()] == nullptr) {
            ...
            board[moveToPlay.getToX()][moveToPlay.getToY()] = move(board[moveToPlay.getFromX()][moveToPlay.getFromY()]->getCreateNew(board[moveToPlay.getFromX()][moveToPlay.getFromY()]->getColour(), board[moveToPlay.getFromX()][moveToPlay.getFromY()]->getPieceType()));
            board[moveToPlay.getFromX()][moveToPlay.getFromY()] = nullptr;
        return true;
        }
        return false;
